ConsensusClusterPlusPlus <- function( d=NULL,
                                  maxK = 3,
                                  reps=10,
                                  pItem=0.8,
                                  pFeature=1,
  ...
  for (tk in 2:maxK){
    ...
    cuct = paste0("C",uct)
    # cuct <- c()
    # for (num in (1:length(uct))){
    #   cuct <- c(cuct,paste0("C",uct[num]))
    # }
    
    
    legend("topright",legend=cuct,fill=unique(colorList[[1]]),horiz=FALSE )
